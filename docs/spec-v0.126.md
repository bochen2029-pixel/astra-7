# ASTRA-7 Foundation Specification v0.126

*The Lock Layer. Third editing pass — bug-fix patch on v0.125. Spinal in commitments. Provisional in particulars.*
*Drafted 2026-05-14, revised same day across three rosetta-stone rounds: v0.123 (first pass), v0.125 (second pass), and v0.126 (this patch, the absolute last pre-Phase-0 revision). v0.124 was discussed across three review threads and skipped in favor of one consolidated v0.125; v0.126 patches six issues caught in post-v0.125 cross-review, one of which (the rapidity-clamp math bug, N1) silently violated the spec's most physically critical tolerance.*
  ...
  AI:
    Mind: conversation history,
          REEL state: ⟨⚠ INLINE PLACEHOLDER. v0.125 includes a minimal field set
                       ...
                       dual-clock retrieval per §3.9), regime_at_write,
                       author_instance_id, body_text, retrieval_metadata,
                       irreversibility_flag: bool}⟩
...
- `docs/spec-v0.125.md` — second editing pass (historical; superseded by v0.126 due to N1 rapidity-clamp math bug and five clarity gaps caught in cross-review)
...
**v0.126 is the absolute last pre-Phase-0 revision.** v0.123 was the first editing pass; v0.125 was the second; v0.126 is a targeted bug-fix patch on v0.125 closing six issues caught in post-v0.125 cross-review — one of them (N1: rapidity-clamp math bug) silently violating the spec's most physically critical tolerance by three orders of magnitude. The next contact the spec needs is with running code, not another editing pass. Phase 0.0 next. v0.2 after that, against measurements. Anything else is the polish trap.

**Discipline acknowledgment:** v0.125 was supposed to be the last revision. It wasn't, because adversarial-mode review caught a numeric primitive that generative-mode review had passed three times. The discipline isn't "ship after one pass"; the discipline is "ship after adversarial review converges to no more real findings." That convergence happened at v0.126, not v0.125. Going forward, every numeric tolerance gets §10's round-trip verification before lock. The discipline strengthens; the iteration count is the cost of the strengthening.
...
>
> *Iterate, don't accumulate. v0.126 today, v0.2 after Phase 0, v1.0 when measurement justifies it.*
>
> *Stop polishing. Start building.*

---

**End of v0.126. Phase 0.0 next.**
