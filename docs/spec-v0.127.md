# ASTRA-7 Foundation Specification v0.127

*The Lock Layer. Fourth editing pass — adds retarded-time observation, cosmological expansion, and the regime-dispatched apparent-rate formula. Spinal in commitments. Provisional in particulars.*
*Drafted 2026-05-14, fourth revision the same day. Iteration history: v0.123 (first pass), v0.125 (second pass), v0.126 (rapidity-clamp + five-clarity patch), v0.127 (this — retarded-time observation, c-bounded epistemology, Observation Calculator).*
*v0.124 was discussed across three review threads and skipped in favor of consolidated v0.125.*
*v0.127 adds the finite-c photon-propagation layer with regime-dispatched apparent-rate (Pass-4-correct), the cosmological-expansion hook, and the Observation Calculator module. Empirically verified against a 528-line C++ binary (48 tests pass) and a Python mirror (45 tests pass); compiled under MSVC v14.16, both implementations converge on identical numbers to 6+ sig figs.*
  ...
  AI:
    Mind: conversation history,
          REEL state: ⟨⚠ INLINE PLACEHOLDER. v0.125 includes a minimal field set
                       ...
                       Provisional fields: list of entries {τ_ship_at_write: float64,
                       t_cosmic_at_write: float64 (NEW v0.126; required for
                       dual-clock retrieval per §3.9),
                       t_emit_event: Optional[float64] (NEW v0.127; for entries
                       referencing observed-distant events — supernovae, stellar
                       transitions, etc. Records when the event actually happened
                       in cosmic-time at the source. Pair with t_cosmic_at_write
                       which records when the light reached the ship. Two-clock
                       memory of distant observations. Distance to source can be
                       reconstructed from c·(t_cosmic_at_write − t_emit_event).),
...
- `docs/spec-v0.126.md` — rapidity-clamp + five-clarity patch (historical; superseded by v0.127 due to missing retarded-time observation layer and the STL_REL longitudinal-Doppler regime-dispatch correction caught in post-v0.126 cross-review)
- `proto/astra_nexus.cpp` — 528-line C++ reference implementation of the unified 14-equation framework (NEW v0.127); 48 assertions; compiles under MSVC v14.16
- `proto/verify_nexus.py` — Python mirror of the same math (NEW v0.127); 45 assertions; both implementations converge to 6+ sig figs on every voyage-demo cell
...
### 15.4 Lock against current findings; revise on new findings; do not polish without findings

**Honest discipline (v0.127 wording).** This document has been declared "the absolute last pre-Phase-0 revision" at v0.123, v0.125, and v0.126 — each declaration wrong because adversarial cross-review surfaced a real finding. The discipline is working; the wording was lying about iteration probability.

**The actual rule is:**

1. Lock against current findings.
2. Revise on new adversarial-review findings (real bugs, missing physics commitments, formula errors, load-bearing gaps).
3. **Do not polish without findings.** Cosmetic changes, prose rewrites, structural reorganization, version-number cleanup — all forbidden absent a real finding.

This is the same discipline that produced v0.125 → v0.126 (the rapidity-clamp N1 bug surfaced; revision justified). And the same discipline that produced v0.126 → v0.127 (the STL_REL vs WARP regime-dispatch error and the missing retarded-time observation layer surfaced; revision justified). Each step had an adversarial-review finding that justified the iteration. Each step's net delta was bug-fix-or-missing-physics, not polish.

Going forward: the iteration count is **the cost of correctness**. If a v0.128 finding surfaces from cross-review with similar rigour, the spec revises. If no finding surfaces, the spec is locked. **The lock is empirical, not declarative.**

What does *not* justify revision:
- "It would be cleaner to reorganize"
- "I want to rename for consistency"
- "Let me elaborate on this paragraph"
- "Let me add a section on X just in case"

What *does* justify revision:
- A compileable round-trip test fails the spec's claim
- A cross-LLM review identifies a real physics or architecture error
- A missing commitment that the next implementation phase would discover anyway
- An empirically verified bug

After v0.127, Phase 0.0 sysprompt validation continues to be the next gating empirical contact. The spec carries no claim about being final; it carries a discipline about how revisions get justified.

**Empirical anchor at v0.127:** the 48-test C++ binary (`proto/astra_nexus.exe`) and 45-test Python mirror (`proto/verify_nexus.py`) lock the regime-dispatched apparent-rate values. If a future finding contradicts those numbers, the binary either updates or the spec revises. The binary IS the lock.
...
>
> *Iterate, don't accumulate. v0.127 today, v0.2 after Phase 0, v1.0 when measurement justifies it. The lock is empirical, not declarative.*
>
> *Stop polishing. Start building.*

---

**End of v0.127. Phase 0.0 next; spec held against new findings.**
