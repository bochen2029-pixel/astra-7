...
| 6 | Next candidates, from the LIVE_RUN routing (operator picks): (a) adapter intent→op normalization (rules-based synonym map; F-LIVE-1/7), (b) heartbeat-coverage expansion to n≥30 before thresholds (F-LIVE-2), (c) Narrator-wired live pass (F-LIVE-5), (d) drill-catch → corpus conversion (F-LIVE-3) | LIVE_RUN §Routing | pending |
