...
- **Perception-variability → repetition coupling** (F-LIVE-30, NEW and open) — the inventory requirement made `<state>` near-identical on quiet heartbeats, and near-identical perception induces near-identical speech (non_degenerate 0.995 → 0.979, established). The right owner is probably NOT the sysprompt: on a tick where nothing changed, the question is what the state section should include at all. Any attempt must A/B against the 6h treatment band [0.970, 0.987].
