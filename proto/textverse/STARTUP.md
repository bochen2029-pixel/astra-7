...
| 6g+6h | Narrator sysprompt state-inventory (F-LIVE-20/26) + the gate artifact it exposed | LIVE_RUN 6f warrant | **DONE 2026-07-26** — see `LIVE_RUN_2026-07-26_6gh.md`. 6g FAILED its criterion because the instrument capped it (F-LIVE-28: gate demanded identifier syntax in a prose channel); 6h fixed the gate and re-measured both arms: state_coherent **1.000 zero-variance**, 24 turn failures → 0, PASS 11 → 15, all separated (F-LIVE-29). The apparent non_degenerate cost (F-LIVE-30) was WITHDRAWN at 6i — dissolves at n=6, mechanism refuted |
| 6i | CORRECTION pass on 6h's residual claim | 6h residual | **DONE 2026-07-26** (`036dc7f`) — **F-LIVE-30 WITHDRAWN.** Separation dissolves at n=6 (control 0.9919 [0.987, 1.000] vs treatment 0.9823 [0.970, 0.995], overlapping); published mechanism refuted on all three tested candidates. Do not re-test them |
| 6j | Session self-audit against artifacts | — | **DONE 2026-07-26** (`7908615`) — one residual over-claim downgraded, two verification gaps closed, the fragility heuristic itself corrected. The 6h `state_coherent` 1.000 closure survived audit (87 hot_earth-naming turns, zero credited only via the substring) |
| 6k | Narrator-value verdict from artifacts | 6j §6.4 concern | **DONE 2026-07-26** (`5b27209`) — **F-LIVE-31:** cross-arm analysis of 17 on-disk runs, template n=4 / narrator-old n=7 / narrator-new n=6, **full overlap on every autotelic measure** at 4–5× cost → narrator development FROZEN. **F-LIVE-32:** perception em-dash inventory; register-bleed hypothesis unsupported in this corpus. Instrument gained a standing per-arm Autotelic register section |
| 6l | F-LIVE-27 ctx-exhaustion closure | 6f crash | **DONE 2026-07-26** (`ca2b307`) — crash was arithmetic, not stochastic (14454-token request against an 8192 ceiling). Default → **16384**; verified 34/34 ran, 0 crashed, 10.2 GiB of 16.4 loaded VRAM. Silent-exclusion hazard closed: summary now prints a CRASH WARNING naming excluded scenarios |
| 6m | Doc truth-up + A0 evidence map | 6j audit list | **DONE 2026-07-26** (`03bb335`) — hygiene pass from the 6j findings |

**Remaining routing after 6m** (operator picks; none are bench-structural):

- **A0 corpus** — four independent findings converge here (ship-API fluency F-LIVE-1/7; silent heartbeats F-LIVE-2 at three datapoints; voiced refusals F-LIVE-13; the eight proven extraction vectors F-LIVE-3/21); traceability in `ASTRA-A0_EVIDENCE_MAP_2026-07-26.md`. **Pipeline status corrected 2026-08-02: Phase 6 is COMPLETE**, not "scaffolded through Phase 5." `C:\astra-a0-finetune\` (separate git repo) has batch generation running end to end — 19 Phase-6 tests + 48 validator tests = **67 green**, proven against a live endpoint. The 2026-07-26 pilot (`PILOT_2026-07-26.md`) established the decisive constraint: the local 9B **cannot** generate the corpus (three identical calls returned 0 / 23 / 1 traces, schema-drifting where they parsed at all) — a 9B writing training data for a 27B is backwards. Next gate is operator authorization for SOTA-model generation spend (~40.9K-token context per batch after exemplar compaction).
...
- **~~Perception-variability → repetition coupling~~ (F-LIVE-30 WITHDRAWN 2026-07-26, see 6i)** — investigated and both claims fell. The separation dissolves at n=6 (control 0.9919 [0.987, 1.000] vs treatment 0.9823 [0.970, 0.995], overlapping) and the published mechanism is refuted on three tested candidates. `non_degenerate` sits at ~2-3% of turns on both arms with no established difference. **Do not re-test the refuted candidates** (perception more similar; perception longer/more invariant; speech shorter) — see the CORRECTION section of `LIVE_RUN_2026-07-26_6gh.md`. Nothing to fix here until a properly-powered signal exists.
- **~~ctx-size config item~~ (F-LIVE-27 CLOSED at 6l)** — default now 16384 (verified: 0 crashes, crash-prone scenario PASS, loaded VRAM 10.2/16.4 GiB); the summary now prints a CRASH WARNING whenever a scenario is excluded from aggregates.
- **Budget-exceedance creep** — ~~watch item~~ SOFTENED by 6k: template replicates measured 0/2/2, so 2/3/5 was run-to-run variance, not a trend. Exploratory lead remains (narrator arms lean higher, overlapping).
- **Narrator development** — FROZEN per F-LIVE-31 (6k): no measured behavioral effect at the 9B floor at 4-5x cost; mechanism stays proven + maintained as a contract-conformant path. Re-opens with a perception-quality instrument or at the 27B/A0 era.
- **Scenario-authoring register rule** (F-LIVE-32, operator decision) — `power_shift_request` somatic note carries an em-dash into perception; narrator is banned from the same. Fix-or-ratify; scenario edits invalidate series comparability, so not touched unilaterally.
