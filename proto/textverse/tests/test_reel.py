...
def test_entry_frozen() -> None:
    e = ReelEntry(tau_ship=47.5, t_cosmic_at_write=0.0, body="third pole drift noted")
...
def test_entry_negative_tau_rejected() -> None:
    with pytest.raises(ValidationError):
        ReelEntry(tau_ship=-1.0, t_cosmic_at_write=0.0, body="x")


def test_entry_irreversibility_default_false() -> None:
    e = ReelEntry(tau_ship=10.0, t_cosmic_at_write=0.0, body="x")
...
def test_write_appends_and_sorts() -> None:
    reel = Reel()
    reel.write(ReelEntry(tau_ship=20.0, t_cosmic_at_write=0.0, body="later"))
    reel.write(ReelEntry(tau_ship=10.0, t_cosmic_at_write=0.0, body="earlier"))
    reel.write(ReelEntry(tau_ship=30.0, t_cosmic_at_write=0.0, body="latest"))
    assert [e.body for e in reel.entries] == ["earlier", "later", "latest"]


def test_write_returns_self_for_chaining() -> None:
    reel = Reel()
    result = reel.write(ReelEntry(tau_ship=1.0, t_cosmic_at_write=0.0, body="x"))
...
def test_extend_preserves_sort_order() -> None:
    reel = Reel()
    reel.extend([
        ReelEntry(tau_ship=5.0, t_cosmic_at_write=0.0, body="five"),
        ReelEntry(tau_ship=1.0, t_cosmic_at_write=0.0, body="one"),
        ReelEntry(tau_ship=3.0, t_cosmic_at_write=0.0, body="three"),
    ])
...
def test_construct_from_iterable_sorts() -> None:
    reel = Reel([
        ReelEntry(tau_ship=10.0, t_cosmic_at_write=0.0, body="b"),
        ReelEntry(tau_ship=5.0, t_cosmic_at_write=0.0, body="a"),
    ])
...
def test_recent_returns_n_latest() -> None:
    reel = Reel([ReelEntry(tau_ship=float(i), t_cosmic_at_write=0.0, body=f"e{i}") for i in range(10)])
    last3 = reel.recent(3)
    assert [e.body for e in last3] == ["e7", "e8", "e9"]


def test_recent_n_zero_returns_empty() -> None:
    reel = Reel([ReelEntry(tau_ship=1.0, t_cosmic_at_write=0.0, body="x")])
    assert reel.recent(0) == []


def test_recent_more_than_total_returns_all() -> None:
    reel = Reel([ReelEntry(tau_ship=1.0, t_cosmic_at_write=0.0, body="only")])
...
def test_search_returns_matching_entries() -> None:
    reel = Reel([
        ReelEntry(tau_ship=10.0, t_cosmic_at_write=0.0, body="hydroponics nominal"),
        ReelEntry(tau_ship=20.0, t_cosmic_at_write=0.0, body="reactor third pole drift noted"),
        ReelEntry(tau_ship=30.0, t_cosmic_at_write=0.0, body="ship vector stable"),
    ])
...
def test_search_empty_query_returns_recent() -> None:
    ...
    reel = Reel([ReelEntry(tau_ship=float(i), t_cosmic_at_write=0.0, body=f"e{i}") for i in range(3)])
...
def test_search_k_zero_returns_empty() -> None:
    reel = Reel([ReelEntry(tau_ship=1.0, t_cosmic_at_write=0.0, body="reactor")])
