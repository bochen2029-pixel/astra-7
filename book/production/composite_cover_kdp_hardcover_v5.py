...
OUT_PDF = OUT_DIR / "cover_wrap_hardcover_v5.pdf"
OUT_JPG = OUT_DIR / "cover_wrap_hardcover_v5.jpg"
...
def render_back(canvas):
    ...
    def render_paragraph(text, style):
        ...
        # v5: also narrow_w wraps META-style at narrower width so URL line
        # ("Steam · GitHub · Hugging Face (AI bundle)") wraps to two lines
        # instead of one long line that extends into the barcode X zone.
        dodge_x = 0
        eff_w = text_w  # default: use full text area width
        if style == "pull_quote":
            font = font_medium(pull_quote_size)
            color = OFF_WHITE
            line_height = int(pull_quote_size * 1.35)
            margin_top = int(pull_quote_size * 0.4)
            margin_bottom = int(pull_quote_size * 0.85)
            align = "center"
        elif style == "body":
            font = font_regular(body_size)
            color = WHITE
            line_height = int(body_size * 1.45)
            margin_top = int(body_size * 0.45)   # v5: tightened from 0.5
            margin_bottom = int(body_size * 0.22)  # v5: tightened from 0.25
            align = "left"
        elif style == "tagline":
            font = font_semibold(tagline_size)
            color = OFF_WHITE
            line_height = int(tagline_size * 1.3)
            margin_top = int(tagline_size * 0.6)
            margin_bottom = int(tagline_size * 0.5)
            align = "center"
            dodge_x = BARCODE_DODGE_PX
        elif style == "italic_phrase":
            font = font_medium(italic_phrase_size)
            color = OFF_WHITE
            line_height = int(italic_phrase_size * 1.35)
            margin_top = int(italic_phrase_size * 0.35)
            ...
            margin_top = int(meta_size * 0.3)  # v5: tightened from 0.4
            margin_bottom = int(meta_size * 0.18)  # v5: tightened from 0.2
            align = "left"
            # v5: narrow meta width so URL line wraps to 2 lines (keeps "AI bundle)"
            # from extending into the barcode X zone at panel right edge).
            eff_w = int(text_w * 0.60)
        elif style == "byline":
            font = font_regular(byline_size)
            color = DIM_WHITE
            line_height = int(byline_size * 1.4)
            margin_top = int(byline_size * 1.0)  # v5: tightened from 1.2
        ...
        lines = wrap_text(text, font, eff_w)
        for line in lines:
...
if __name__ == "__main__":
    print("=== KDP HARDCOVER WRAP COMPOSITE v5 — The Long Watch ===")
    print(f"  v5: META lines wrap at 60% text_w (URL line forced to 2 lines, clears barcode X)")
    print(f"      + tightened paragraph spacing more (URL line moves up, clears barcode Y)")
    print(f"      + tagline/byline still dodged {BARCODE_DODGE_PX}px left (from v4)")
    print(f"  Spine: {SPINE_IN}\" (unchanged from v3/v4)")
    make_wrap()
    print("\nDone.\n")
